...
export default function DomeGallery({
  ...
  useEffect(() => {
    ...
    const ro = new ResizeObserver(entries => {
      ...
      if (enlargedOverlay && frameRef.current && mainRef.current) {
        ...
        if (hasCustomSize && window.innerWidth > 768) { // Only apply custom size on desktop
          ...
          enlargedOverlay.style.width = tempRect.width + 'px';
          enlargedOverlay.style.height = tempRect.height + 'px';
        } else {
  ...
  const startAutoRotate = useCallback(() => {
    if (autoRotateRef.current || autoRotateSpeed === 0) return;
    const step = () => {
  ...
  const startInertia = useCallback(
    ...
    [dragDampening, maxVerticalRotationDeg, stopInertia, applyTransform, startAutoRotate]
  );
        ...
        if (last) {
          ...
          if (!isTap && (Math.abs(vx) > 0.005 || Math.abs(vy) > 0.005)) {
            startInertia(vx, vy);
             ...
             startAutoRotate(); // Resume auto-rotate if not starting inertia
          }
  ...
  }, [enlargeTransitionMs, openedImageBorderRadius, grayscale, startAutoRotate]);
  ...
  const openItemFromElement = (el: HTMLElement) => {
    ...
    const wantsResize = (openedImageWidth || openedImageHeight) && window.innerWidth > 768; // Only resize on desktop
    if (wantsResize) {
