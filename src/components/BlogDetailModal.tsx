...
import { sanitizeHtml, isSafeUrl, sanitizeUrl } from '@/lib/security';
...
export const BlogDetailModal = ({ post, open, onOpenChange }: BlogDetailModalProps) => {
  ...
  // Split content into paragraphs for clean typography and sanitize each paragraph
  ...
  const safeImageUrl = post.image_url && isSafeUrl(post.image_url) ? post.image_url : null;
  const rawSourceUrl = post.source_url
    ? post.source_url.startsWith('http')
      ? post.source_url
      : `https://${post.source_url}`
    : '';
  const safeSourceUrl = isSafeUrl(rawSourceUrl) ? rawSourceUrl : null;

  const sanitizedSnippet = post.snippet ? sanitizeHtml(post.snippet) : '';
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="bg-[#141414] border-gray-800 text-white max-w-2xl max-h-[90vh] overflow-y-auto p-0 gap-0 rounded-2xl">
          ...
          <div className="relative w-full h-56 md:h-72 overflow-hidden bg-gray-900">
            ...
            <div className="absolute top-4 left-4 flex gap-2">
              ...
              {post.read_time && (
                <Badge
                  variant="secondary"
                  className="bg-black/60 backdrop-blur-md text-gray-300 border-gray-700 text-xs gap-1"
                >
        ...
        <div className="p-6 md:p-8 space-y-6">
          ...
          {post.content && post.snippet && post.snippet !== post.content && sanitizedSnippet && (
            <div
              className="p-4 rounded-xl bg-gray-900/80 border border-gray-800 text-gray-300 text-sm font-medium leading-relaxed italic border-l-4 border-l-[#39FF14]"
